...
def main():
    ...
    rotationCalculator = screenRotation()
    angle = 0
    ...
    def render_callback(env_renderer):
        ...
        e.top = top + 800
        e.bottom = bottom - 800
        if rotationCalculator.pc % 2 == 0 :
            e.rotation = rotationCalculator.getRotation(angle)
        rotationCalculator.pc += 1
...
class screenRotation:
    """
    For calculating the screen rotation 
    """
    def __init__(self):
        self.loops = 60
        self.rotationArray = [0] * self.loops
        self.pc = 0
        self.rounds = 0


    def getRotation(self, theta):
        currentRotation = (-1 * theta) + 1.7 + self.rounds

        if (abs(currentRotation - self.rotationArray[1]) > 3):
            if (abs(currentRotation + 6.28 - self.rotationArray[1]) < 3):
                self.rounds += 6.28
                currentRotation += 6.28
            else:
                self.rounds -= 6.28
                currentRotation -= 6.28


        self.rotationArray.insert(0, currentRotation)
        i = 0
        sum = 0
        while i <= self.loops:
            sum += self.rotationArray[i] * (1/self.loops)
            i += 1
